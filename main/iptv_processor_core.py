...
# 兼容旧版config.py（核心修复：若config中缺少对应属性，自动补充默认值，避免报错）
if not hasattr(config, "ALLOW_REDIRECTS"):
    config.ALLOW_REDIRECTS = CONFIG_DEFAULTS["ALLOW_REDIRECTS"]
if not hasattr(config, "MAX_REDIRECTS"):
    config.MAX_REDIRECTS = CONFIG_DEFAULTS["MAX_REDIRECTS"]
...
备注2：已修复「config无ALLOW_REDIRECTS/MAX_REDIRECTS属性」报错，双保险适配：① 脚本中添加属性检测，缺失自动补充默认值；② 配置文件中明确添加该属性，彻底避免报错。

备注3：若想进一步提升成功率，建议在本地电脑运行（而非GitHub Actions），本地IP更难被封禁，成功率可再提升30%+。
